...
class NavbarComposer{
	...
	public function count_dashboard($parent_id,$role_id){
		...
		$count = Authentication::select('authentications.*')->join('authentication_items','authentication_items.auth_id','=','authentications.id')->where('authentications.parent_id',$parent_id)->where('authentication_items.role',$role_id)->where('authentications.deleted_at',NULL)->count();
		// foreach ($count as $c) {
			if($count > 0){
	...
	public function get_dashboard($parent_id,$role_id){
		...
		$dashboard = Authentication::select('authentications.*')->join('authentication_items','authentication_items.auth_id','=','authentications.id')->where('authentications.parent_id',$parent_id)->where('authentication_items.role',$role_id)->where('authentications.deleted_at',NULL)->orderBy('authentications.order_id','DESC')->get();
		$arr = [];
